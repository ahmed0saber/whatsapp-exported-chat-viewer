    ...
    for(let i=0; i<allMessages.length; i++){
        for(let j=0; j<allMessages[i].length; j++){
            if(dataNumber == 0 && allMessages[i][j] == "-"){
                if(currentData.slice(0, 10).match(/^\d{1,2}([./-])\d{1,2}\1\d{2,4}$/) || 
                currentData.slice(0, 9).match(/^\d{1,2}([./-])\d{1,2}\1\d{2,4}$/) || 
                currentData.slice(0, 8).match(/^\d{1,2}([./-])\d{1,2}\1\d{2,4}$/) || 
                currentData.slice(0, 7).match(/^\d{1,2}([./-])\d{1,2}\1\d{2,4}$/) || 
                currentData.slice(0, 6).match(/^\d{1,2}([./-])\d{1,2}\1\d{2,4}$/)){
